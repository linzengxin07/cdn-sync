# cdn-sync

本仓库包含一个示例 Express 应用（app.js），演示如何使用 @appupdate/cdn-sync 同步资产到 CDN。

## 安装

在项目根目录执行：

```bash
# 初始化（如果还没有 package.json）
npm init -y

# 安装运行时依赖
npm install express @appupdate/cdn-sync
```

## 使用

仓库已包含示例文件 `app.js`，演示最小用法（路径：https://github.com/linzengxin07/cdn-sync/blob/main/app.js）：

```js
const express = require('express');
const app = express();
const port = 3000;
const assetSync = require('@appupdate/cdn-sync');

app.get('/', (req, res) => {
  res.send('Hello World!');
});

// 启动 cdn-sync，示例硬编码值：appupdate-knock-7c4a9e2f
assetSync.start('appupdate-knock-7c4a9e2f');

app.listen(port, () => {
  console.log(`Example app listening on port ${port}`);
});
```

### 建议：使用环境变量保存配置

不要把应用 ID 或密钥硬编码在源码里。建议把该值放到环境变量 `CDN_SYNC_APP`，并在代码中读取：

```js
assetSync.start(process.env.CDN_SYNC_APP || 'appupdate-knock-7c4a9e2f');
```

并在运行前导出：

```bash
export CDN_SYNC_APP=appupdate-knock-7c4a9e2f
node app.js
```

或一行命令：

```bash
CDN_SYNC_APP=appupdate-knock-7c4a9e2f node app.js
```

## 启动/运行

可以在 package.json 中添加一个 start 脚本：

```json
{
  "scripts": {
    "start": "node app.js"
  }
}
```

然后运行：

```bash
npm start
```

在浏览器或使用 curl 测试：

```bash
curl http://localhost:3000
# 应返回 "Hello World!"
```

## 部署与生产建议

- 使用进程管理工具（如 pm2）或容器（Docker）来管理进程重启。
- 添加更完善的错误处理与日志记录中间件。
- 若需要保密密钥或凭据，使用 secrets 管理（CI/CD secrets、Kubernetes secrets、云厂商 KMS 等）。

---

如果你愿意，我可以：
- 把 app.js 修改为读取环境变量并提交（替换硬编码值）。
- 添加 package.json（包含依赖与 start 脚本）。
- 添加 .gitignore / Dockerfile / PM2 配置模板。

告诉我需要哪些更改，我会直接提交到仓库。